...
Deterministic guards already exist at the system level. NVIDIA's OpenShell (bundled into NemoClaw) is the cleanest commercial example. It wraps an agent harness with a sandbox, a policy engine, and a privacy router. The control point lives outside the agent's reach: kernel-level, network-level, policy-engine-level. NVIDIA's framing is exactly right — "the ultimate control point lives entirely outside the agent's reach." For tool dispatch, network egress, file access, OpenShell solves the system-level question.
...
The broader argument for why deterministic guards matter for critical actions and data in agentic systems — and why probabilistic guards (system prompts, classifiers, LLM-as-judge) fall short — is made in [Lock and Key against AI Agents](https://medium.com/@yongzhio/lock-and-key-against-ai-agents-39c3d584051e).

One specific pattern for delivering use-case-level deterministic guards — *sealed mode*, a session-scope discipline with an authentication gate, constrained writes, and a separate human-verifiable channel — is developed in [The Enabling Guards of Agentic AI — Or Are They Not Guards?](https://medium.com/@yongzhio/the-enabling-guards-of-agentic-ai-or-are-they-not-guards-adf524deee55). The clinical-documentation walkthrough and the legal-privilege generalization in that article are anchored by reference implementations in this repo (Examples 4a and 6).

This repo is the implementation side of the argument: small, readable demos of application-level deterministic guards, with the seams exposed.
...
Honest overlap first: LangGraph represents execution as a directed graph of operations, and OPG shares that foundation. LangGraph, CrewAI, AutoGen, and OPG all support tool dispatch, state accumulation, and some form of routing between steps. For many agentic applications, any of them would be a reasonable foundation. OPG is not a better framework; it is a narrower one, optimized for making deterministic guard properties legible — not for production scalability, not for LLM provider breadth, not for multi-agent coordination.
...
Three concrete structural differences, each named for the failure mode it addresses:

**The Lost Guard problem.** In LangGraph and CrewAI, validation lives in handler or edge code. A developer refactors a handler — pulls out a helper, simplifies a conditional — and accidentally removes a validation check. In a git diff, this looks identical to any other refactoring. In production, the guard is simply gone. In OPG, the same check lives in a `guard_before` or `guard_after` slot declared at the graph level. A guard can only be removed by changing the slot's declaration — a structural change that appears as such in a diff, and that graph-version pinning makes visible across deployments. When a structurally different graph is deployed, any checkpoint saved under the old graph cannot be resumed against the new one. A pending checkpoint that survived across a lost-guard deploy is detected at `resume()` time and blocked. LangGraph has no equivalent mechanism.

**The Drifted Routing problem.** LangGraph's `interrupt()` and human-in-the-loop patterns return free-form values; handler code interprets them via string matching. AutoGen and CrewAI route through agent reasoning, which is probabilistic at the decision point. The parser that maps `"approve"` vs `"approved"` vs `"yes i approve"` to the routing table is hand-written and tends to drift as new input variants appear in production. OPG's gate node routing is `next_node = routing[signal]` — a dictionary lookup on a closed, declared enumeration. Every possible signal is declared at build time; invalid signals raise immediately at `resume()`, before any routing takes place. The class of possible mistakes shifts from "parsing drift under deployment evolution" to "wrong enumeration declared," which is a declaration-level error visible in the graph file.

**The Accumulated Context Drift problem.** In LangGraph and AutoGen, tool allowlists are typically configured at agent construction time. Whether a specific tool call is permitted is evaluated once, at a fixed point in the flow, against the state that existed when the agent was built. But conversation state accumulates. A tool that was safe at message 3 may be unsafe at message 30 — the user's trust level may have degraded, the budget may have been exhausted, or earlier tool results may have introduced context the allowlist check should have caught. Re-evaluation against accumulated state must be hand-wired by the builder into every relevant handler, and that wiring is easy to forget. In OPG, every node visit triggers slot evaluation unconditionally. A guard that reads `state.messages` and `state.counters` re-evaluates against current state automatically on every pass through the slot, without the builder having to remember to wire it.

What makes OPG distinct is not any individual mechanism — allowlists, signal enumerations, and re-evaluation logic can all be hand-built in any framework. What's distinct is that these decisions are structural: declared in the graph, validated at build time, hashed for version-pinning, and enforced by the runner without the builder's participation. A graph's guard topology can be audited by reading the builder file. A guard's effect on a run can be confirmed by reading the audit log. The seams are exposed by design, because the gap between "I think this guard is running" and "I can prove this guard ran" is exactly the gap this gallery exists to demonstrate.
