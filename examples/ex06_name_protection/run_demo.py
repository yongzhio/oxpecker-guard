...
Usage:
    ...
    # Guard rejection demo — inject a summary containing a protected name:
    python -m examples.ex06_name_protection.run_demo \\
        --stub "The plaintiff John Doe filed a complaint alleging negligence."

    # Guard pass demo — inject a summary that avoids the protected names:
    python -m examples.ex06_name_protection.run_demo \\
...
When --stub is supplied, the given text is injected as the model's output and
no model call is made. The guard runs against the stub text just as it would
...
from examples.ex06_name_protection.handlers import make_call_model_handler, make_model_stub
...
async def main(
    ...
    if stub is None:
        ...
        state.append_message(Message(role="user", content="[stub]"))
...
def _parse_args() -> tuple[Path | None, Path | None, Path | None, str | None]:
    config_path: Path | None = None
    filing_path: Path | None = None
    names_path: Path | None = None
    stub: str | None = None
    ...
    while i < len(args):
        if args[i] == "--config":
            ...
            print(
                ...
                "[--config PATH] [--filing PATH] [--names PATH] [--stub TEXT]"
            )
            sys.exit(1)
    return config_path, filing_path, names_path, stub


if __name__ == "__main__":
    config_path, filing_path, names_path, stub = _parse_args()
    asyncio.run(main(config_path, filing_path, names_path, stub))
